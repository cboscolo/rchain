...
def test_propose(container):
    ...
    for i in range(1, args.propose_loop_amount+1):
        ...
        cmd = 'for i in `ls /opt/docker/examples/*.rho | sort -R | tail -n 3`; do echo "running deploy with ${i}"; /opt/docker/bin/rnode deploy --from "0x1" --phlo-limit 100000 --phlo-price 0 --nonce 0 ${i}; done'
        try:
            r = container.exec_run(['sh', '-c', cmd])
    ...
    while True:
        for container in client.containers.list(all=True, filters={"name":f"peer\d.{args.network}"}):
            for i in range(1, args.propose_loop_amount+1):
                ...
                # Deploy example contracts using 3 random example files
                cmd = 'for i in `ls /opt/docker/examples/*.rho | sort -R | tail -n 3`; do /opt/docker/bin/rnode deploy --from "0x1" --phlo-limit 100000 --phlo-price 0 --nonce 0 ${i}; done'
                try: 
                    r = container.exec_run(['sh', '-c', cmd])
                    for line in r.output.decode('utf-8').splitlines():
                        print(line)
